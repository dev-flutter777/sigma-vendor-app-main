  ...
  void _addBasicProductFields(
    ...
    fields.addAll({
      ...
      'brand_id': Provider.of<SplashController>(Get.context!, listen: false).configModel!.brandSetting == "1"
          ? product.brandId
          : null,
      'brand_name': addProduct.brandName,
    ...
    // Optional measurements must be omitted when blank. Sending null/empty
    // multipart values can be serialized as the strings "null" or "0" by
    // different clients, which would corrupt the server-side product record.
    const optionalMeasurements = <String>{
      'length', 'width', 'height', 'dimension_unit', 'weight', 'weight_unit',
    };
    fields.removeWhere((key, value) => optionalMeasurements.contains(key) &&
